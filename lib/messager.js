...
module.exports.msg = function (browser, message, error, msgDelay, pauseOnError) {
    if (error) {
        element(by.css('input[id="fisMessagerErrorMessageInput"]')).clear().sendKeys(message);
        // this should be configurable (maybe screenshot errors also...)
        if (pauseOnError) {
            browser.pause();
        }
    } else {
        element(by.css('input[id="fisMessagerMessageInput"]')).clear().sendKeys(message);
...
module.exports.repositionMessager = function (position) {
    if (position && position !== '') {
        element(by.css('input[id="fisMessagerPosition"]')).clear().sendKeys(position);
    }
};
