 ...
 * @param options plain object containing settings whether log oder csv file should be created
 *              Default options object: {writeLogFile: true, writeCsvFile: true}
 */
    ...
    return (function () {
        ...
        return {
            measureMemory: function(isLast){
                return module.exports.measureMemory(isLast ? initialResult : memoryResult, isLast, filename, csvFilename, options, graphData).then(function (result) {
...
module.exports.runTestFunction = function(test, iterations, testFn, options) {
    ...
    if (options.generateGraph) {
       graphData = [{name: 'usedJSHeapSize', values: []},{name: 'totalJSHeapSize', values: []}];
    }
    ...
    memoryMeasure.callGarbageCollection().then(function () {
        ...
        memoryMeasure.measureMemory(true).then(function() {
            if (graphFilename && options.generateGraph) {
                generateGraph.generateGraphImage(graphFilename, graphData, options.graphWidth, options.graphHeight);
            }
        });
    });
 ...
 */
module.exports.measureMemory = function (comparisonMeasurement, isLast, filename, csvFilename, options, graphData) {
    options = options || {};
    ...
    browser.driver.executeScript(function () {
        ...
        if (!comparisonMeasurement) {
            ...
            if (isLast) {
                ...
                if (options.writeLogFile) {
                    logResults(buffer, '-----Comparing memory sizes ---- ');
                }
            }
            ...
            if (options.writeLogFile) {
                logResults(buffer, 'Heap limit change: ' + (fl > 0 ? '+' : '') + fl + ' (now at ' + result.jsHeapSizeLimit + ')');
                logResults(buffer, 'Used heap change ' + (fh > 0 ? '+' : '') + fh + ' (now at ' + result.usedJSHeapSize + ')');
                logResults(buffer, 'Total heap change: ' + (ft > 0 ? '+' : '') + ft + ' (now at ' + result.totalJSHeapSize + ')');
		...
		if (options.generateGraph) {
			...
			graphData[0].values.push(result.usedJSHeapSize);
            graphData[1].values.push(result.totalJSHeapSize);
        }
